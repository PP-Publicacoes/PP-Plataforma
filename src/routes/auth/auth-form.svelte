...
<Tabs.Root value={data.tab}>
	...
	<Tabs.Content value={AuthTab.login}>
		<form method="POST" use:enhanceLogin action="?/{AuthTab.login}" class="flex flex-col gap-1">
			...
			<Form.Field form={loginForm} name="password">
				<Form.Control>
					{#snippet children({ props })}
						...
						<Input {...props} bind:value={$loginFormData.password} type="password" />
					{/snippet}
				</Form.Control>
			...
			<div class="mt-2 text-center text-sm">
				{m['login.footer']()}
				<a href="?t={AuthTab.register}" class="underline underline-offset-4"
					>{m['auth_tab.register']()}</a
				>
			</div>
		</form>
	</Tabs.Content>
	<Tabs.Content value={AuthTab.register}>
		...
		>
			...
			<Form.Field form={registerForm} name="email">
				<Form.Control>
					{#snippet children({ props })}
						...
						<Input {...props} bind:value={$registerFormData.email} />
					{/snippet}
				</Form.Control>
			...
			<Form.Field form={registerForm} name="password">
				<Form.Control>
					{#snippet children({ props })}
						...
						<Input {...props} bind:value={$registerFormData.password} type="password" />
					{/snippet}
				</Form.Control>
			...
			<Form.Field form={registerForm} name="confirmPassword">
				<Form.Control>
					{#snippet children({ props })}
						...
						<Input {...props} bind:value={$registerFormData.confirmPassword} type="password" />
					{/snippet}
				</Form.Control>
			...
			<div class="mt-2 text-center text-sm">
				{m['register.footer']()}
				<a href="?t={AuthTab.login}" class="underline underline-offset-4">{m['auth_tab.login']()}</a
				>
			</div>
		</form>
	</Tabs.Content>
