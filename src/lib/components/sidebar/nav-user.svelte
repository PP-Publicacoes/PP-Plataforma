		...
		<Sidebar.MenuItem>
			<DropdownMenu.Root>
				...
				>
					...
					<DropdownMenu.Separator />
					<Button variant="ghost" class="w-full justify-start" onclick={goToProfile}>
						<UserRound />
						{m['nav_user.profile']()}
					</Button>
					<!-- <DropdownMenu.Item> -->
					...
					<!-- 	Billing -->
					<!-- </DropdownMenu.Item> -->
					<Button variant="ghost" class="w-full justify-start p-1" onclick={goToNotifications}>
						<BellIcon />
					...
					<form method="POST" action="/auth?/logout" class="m-0">
						...
						<Button type="submit" variant="ghost" aria-label="Log out" class="w-full justify-start">
							<LogOutIcon />
							{m['logout.label']()}
						</Button>
					</form>
